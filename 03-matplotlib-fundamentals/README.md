...
## Real Space ML Examples

### Example 1: Satellite Cloud Monitoring
...
plt.plot(days, sentinel, color='red', marker='o', label='Sentinel-2')
plt.plot(days, landsat, color='blue', marker='s', label='Landsat-8')
plt.plot(days, goes, color='green', marker='^', label='GOES-16')
plt.xlabel("Days")
plt.ylabel("Cloud Coverage (%)")
plt.title("Weekly Cloud Coverage Comparison")
plt.legend()
plt.grid(axis='y', alpha=0.3)
plt.savefig("satellite_comparison.png")
```

**Insight:** Shows which satellite has clearest observations — critical for mission planning

### Example 2: Vegetation vs Cloud Relationship

```python
ndvi = [0.3, 0.6, 0.8, 0.4, 0.9, 0.5, 0.7]
cloud = [80, 60, 20, 75, 10, 65, 30]

plt.scatter(ndvi, cloud, color='orange', marker='*', s=150)
plt.xlabel("NDVI (Vegetation Index)")
plt.ylabel("Cloud Coverage (%)")
plt.title("Vegetation vs Cloud Cover")
plt.savefig("ndvi_cloud.png")
```

**Insight:** High vegetation occurs when skies are clear — real satellite observation pattern
...
# Panel 1: Trend
axes[0].plot(days, satellite_data, color='red', marker='o')
axes[0].set_title("Temporal Trend")
axes[0].set_ylabel("Cloud Coverage (%)")

# Panel 2: Distribution
axes[1].hist(satellite_data, bins=5, color='blue')
axes[1].set_title("Data Distribution")
axes[1].set_ylabel("Frequency")

# Panel 3: Prediction accuracy
axes[2].scatter(predicted, actual, color='green', marker='s')
axes[2].set_title("Model Performance")
axes[2].set_ylabel("Actual")
axes[2].set_xlabel("Predicted")

plt.savefig("analysis_panel.png")
```

**Insight:** Complete analysis in one view — trend, distribution, accuracy

---

## Learning Progression

1. **Start:** Understand plot types and when to use each
2. **Build:** Add colors, line styles, markers for distinction
3. **Enhance:** Add labels, titles, legends for clarity
4. **Master:** Combine multiple subplots with full styling
5. **Apply:** Use visualization to understand real satellite data

---

## Next Steps

- **After Matplotlib:** Math for ML (Linear Algebra, Statistics, Calculus)
- **Then:** Scikit-learn (classical ML algorithms)
- **Then:** Geospatial Python (Rasterio, GeoPandas) — real satellite work begins
- **Finally:** Real projects for portfolio + internship applications

Matplotlib is visualization tool. Combined with math + ML + geospatial skills = ready for space-tech companies.
...
| File | What It Covers |
|------|----------------|
| `01_line_plot.py` | Basic line plots, clearing figures between plots |
| `02_scatter_plot.py` | Scatter plots, revealing relationships |
| `03_bar_chart.py` | Bar charts for category comparison |
| `04_histogram.py` | Histograms, data distribution, grid lines |
| `05_multiple_plots.py` | Multiple plot types in subplots |
| `06_mixed_subplots.py` | Combining line, scatter, histogram |
| `07_colors_and_styling.py` | Colors, line styles, markers |
| `08_legends_and_labels.py` | Legends and axis labels |
| `09_styled_subplots.py` | Everything combined professionally |
| `.png files` | Saved outputs from each script |

**Review progression:** Start with 01, work through 09. Each builds on previous concepts.